  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Typography variant="h4" fontWeight={600}>API Usage</Typography>
      ...
      {loading && !summary ? (
        ...
        <>
          <Paper sx={{ p: 3, mb: 3 }}>
            ...
            <ResponsiveContainer width="100%" height={280} role="img" aria-label="Bar chart showing API requests over time">
              <BarChart data={usage}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="date" tickFormatter={(d) => new Date(d).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })} />
                <YAxis />
                <Tooltip />
                <Bar dataKey="requests" fill={CHART_PALETTE[0]} radius={[4, 4, 0, 0]} />
                <Bar dataKey="errors" fill={CHART_PALETTE[4]} radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </Paper>
          ...
          <Paper sx={{ p: 3 }}>
            ...
            <ResponsiveContainer width="100%" height={220} role="img" aria-label="Line chart showing API success rate">
              <LineChart data={usage.map((d) => ({
                ...
                <Tooltip />
                <Line type="monotone" dataKey="rate" stroke={CHART_PALETTE[1]} strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </Paper>
        </>
